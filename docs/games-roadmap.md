...
- 2026-09-06 · Phase 12: whole-console backup (`.aspbackup`). One downloaded file carries the library, its saves, PS2 memory cards, player-supplied BIOS dumps, photos, profiles and every localStorage key; restore is behind a confirm step naming what the file holds. Two scopes ("saves & settings" vs "everything"), both sized in a single walk and shown on the buttons. Replaces the two partial exports as the primary path — the setup link and the EmulatorJS folder move under "Share Just Your Setup" in Labs. Binary is stored as zip entries rather than base64 in the manifest, so size scales with the data instead of inflating by a third into one giant string. Also fixes an existing bug: `restoreDb` rebuilt object stores without their indexes, so a restored `asp-games` had no `profileId` index and `listGames()` would throw `NotFoundError` on the next read — that repaired the folder path too. Verified in a browser end to end (seed with real blobs → export → wipe → restore → indexes present, index query returns the game, blobs byte-identical; random bytes and a foreign zip refused with the library intact). Not yet deployed.
- 2026-09-06 · **Phase 12 deployed to production** (main `1988352`). Two bugs were caught between the preview and the deploy, both by unzipping a backup the preview itself produced rather than by the tests: (1) `restoreDb` deleted the whole database and rebuilt it from the dump, so importing a saves-only backup — which carries `asp-games/saves` and `asp-games/bios` and skips `roms`/`photos` — would have erased the game library and photo album, and `asp-rpgm` appeared with zero stores and would have been recreated empty. Restore is now store-by-store: carried stores are cleared and refilled, missing stores are created on a version bump with their indexes, uncarried stores are untouched, and `buildBackup` drops databases whose every store was skipped. (2) The confirm step warned that restoring replaces games for every file, which is false for the saves scope; the warning is now per-scope and the counts pluralise. Regression pass: 31 routes error-free on preview and on production, NES boot 0.817 on both (identical to the pre-deploy baseline — the harness reads `picture=none` unless the Controls card is dismissed first). Backup round-trip driven through the deployed UI on production: seeded a game, photo and save, exported saves-only, altered the save, restored — game and photo still present with the `profileId` index intact and the save reverted.
- 2026-09-09 · Phase 13: PS2 game compatibility + per-game overrides. **Deployed the catalog** (main `2c486a7`): `scripts/ps2compat.mjs` pulls Play!'s tracker (2,610 titles with a state — 1,302 playable, 978 ingame, 46 loadable, 284 intro) into `src/data/ps2compat.ts` (152 KB); `Ps2CompatSheet` sits beside the engine picker reusing `hz-sheet` + `hz-fit` so no new CSS or vocabulary. Filter-first (2.6k rows will not paint; box takes focus, list caps at 60). Verified live: searching "batman" ranks prefix matches first and labels Batman Begins "intro only".
  · **Disc identification costs 6 KB.** `readTitleId()` in `src/ps2compat.ts` walks ISO9660 properly — PVD at 0x8000 → root directory → SYSTEM.CNF → BOOT2 line — so it is three 2 KB reads whether the disc is 700 MB or 4 GB, over the same `Blob.slice` path the emulator streams through. Verified on three real discs (2.35/3.86/0.68 GiB) and the states matched our own hands-on testing exactly.
  · **Per-game overrides** (`src/ps2knobs.ts` + `functions/api/ps2cfg.ts`, KV `ps2cfg:v1` with a `:prev` backup): shipped defaults answer offline, a KV table merges on top so a fix needs no deploy. Everything is re-validated client-side because a bad clock value is a black screen with nothing in the log. Applied per disc at boot: clock and GameConfig knobs (engine/res are fixed at mount — changing them re-points the iframe).
  · **Play!'s GameConfig mechanism, decoded from source + observation.** Single file at `GetAppResourcesPath()/GameConfig.xml`, which in the emscripten build resolves to **`/usr/local/share/GameConfig.xml`** — found by hooking `FS.open` in `public/play-tune/` (test-only harness, noindex) and seeing Play! probe that path on every boot whether or not the file exists. `<GameConfig Executable="…">` matches the **boot ELF name** (`SLUS_211.98`), not the tracker id. Five knobs: `Patch` (Address+Value), `BlockFpRoundingMode` (Address+Mode NEAREST/PLUSINFINITY/MINUSINFINITY/TRUNCATE), `BlockFpUseAccurateAddSub` (Address), `IdleLoopBlock` (Address, optional CheckBlockKey), `Vu1BlockNoClamping` (BlockKey `%08X%08X%08X%08X;%d`). The fork's boot page now writes the XML before `bootDiscImage`.
  · **PROVEN:** the file lands at the path Play! opens, with correct contents, before boot. **NOT PROVEN:** that Play! acts on it — its XML parse errors go through CLog, not `printErr`, so deliberately broken XML produces no observable difference in this build. Confirming needs a knob whose effect is visible, or a build with logging enabled.
  · **Urban Reign regression, root-caused:** not the engine (works on both), not multitaps (works at 1 and 6 players), not size. **Any EE underclock breaks it** — full clock plays, half and third give a black screen with zero errors. Shipped as the first override. Likely trigger: the Phase 12 backup restore rewrites *every* localStorage key including `asp.ps2.eeclock`. Consider excluding emulator settings from restore, or listing them in the confirm step.
  · **Batman Begins is a renderer limitation, not a tunable.** Control experiment against Urban Reign: Batman issues 9,829 draws then freezes; Urban Reign passes 480,000 and climbs. `texParameter: no texture bound to target` is noise (the *working* game emits more, 235 vs 186). The unique signal is **34× `Feedback loop formed between Framebuffer and active Texture`, zero in the control** — the game renders to a framebuffer while sampling it as a texture, which real hardware allows and WebGL forbids, so the draws are dropped and it hangs. No GameConfig knob touches the renderer. Upstream tracks it `state-intro` since 2019 (issue #922) and it fails natively too. A real fix means breaking the feedback loop in the WebGL backend (copy to a scratch texture before sampling) — bounded, in our fork, and would help other games.
  · **Unreported upstream bug found:** on the `native` engine Batman Begins throws `WebAssembly.Module(): Compiling function #0 failed: expected 0 elements on the stack for fallthru, found 1 @+151` 242 times. Root cause in `Play--CodeGen/src/Jitter_CodeGen_Wasm.cpp`: `Emit_CondJmp_AnyAny` pushes a comparison and emits no branch (`INST_BR_IF` appears nowhere in the file); the value is only consumed if the next label is classified `LABEL_FLOW_IF` by a three-pattern matcher, and is orphaned otherwise. Release builds ship it because the validator is behind `#ifdef _DEBUG` ("Only validate in debug"). Zero hits for the error across both repos. Maintainer merges 78% of PRs, median 35 h, with precedent (#1332) of fixing a browser-only WASM bug in 3 days — worth filing with our repro.
- 2026-09-09 · Phase 13 continued: **deployed** the per-game override system (main `e6dddc0`); `/api/ps2cfg` verified live (GET returns the empty table, POST without the admin key gets 403).
  · **Fixed the fork's wasm build, which was completely broken.** libchdr vendors zlib, whose CMakeLists gives both its shared and static targets `OUTPUT_NAME "z"`; Emscripten has no shared libraries so both collapse to `libz.a` and ninja refuses two rules for one output — configure failed before compiling anything. Not avoidable via the preset, single-config Ninja, `BUILD_SHARED_LIBS=OFF` or `WITH_SYSTEM_ZLIB=ON`, because the two targets are defined unconditionally. Fix is three guarded lines in `Source/CMakeLists.txt` renaming the unused shared target's output. Fork branch `fix/webgl-framebuffer-feedback` (`9b9ac6b5`). **Also note: `emcmake cmake -S . -B <dir> -G Ninja -DBUILD_PSFPLAYER=OFF -DBUILD_TESTS=OFF -DUSE_QT=OFF` then `ninja Play` is a working recipe; artifacts land in `build_wasm_sc/Source/ui_js/Play.{js,wasm}`.**
  · **Wrote and shipped a real renderer fix, which works but is not sufficient.** `SearchTextureFramebuffer` handed the shader `framebuffer->m_texture` even when that framebuffer was the current draw target; WebGL rejects that as a feedback loop and discards the draw. Now blits into a scratch texture first (allocated once, grown to the largest request; narrow check so sampling a *different* framebuffer keeps the zero-copy path). **Verified on Batman Begins: feedback-loop errors 34 → 0, framebuffer binds 7,282 → 20,148 (the added blits).** Keep it — render-to-self is common on PS2.
  · **But Batman Begins still hangs**, at exactly 9,829 draws and a black screen, the same count and point as before. A second, independent blocker somewhere in EE/VU/GS emulation, consistent with the game failing on native builds too where no WebGL restriction applies. Do not re-chase this from the renderer side.
  · Gotcha for future work: `Framework::OpenGl::CResource` has `operator GLuint()` and **no** `IsEmpty()` — compare the handle against 0.
