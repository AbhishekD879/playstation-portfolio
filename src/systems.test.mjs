...
assert.deepEqual(classifyFile("game.tic"), { core: "tic80" }, "a TIC-80 cart is itself anywhere");
assert.deepEqual(classifyFile("watris.wasm"), null, ".wasm is a WASM-4 cart only from the fantasy shelf");
assert.deepEqual(classifyFile("watris.wasm", ["tic80", "wasm4"]), { core: "wasm4" });
for (const id of ["tic80", "wasm4"]) assert.ok(SYSTEMS[id].engine === "frame" && SYSTEMS[id].frame?.startsWith("/"), `${id}: frame engine needs a player page`);
