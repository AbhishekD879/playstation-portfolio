// Shared pi.dev (pi-agent-core) bridge core. Both brain backends — WebLLM
// (piWebllm.ts) and Chrome's built-in Gemini Nano (piGemini.ts) — feed a plain
// async stream of text deltas into makeStream(), which emits pi's event stream
// and parses the Hermes <tool_call> JSON convention into typed ToolCall blocks.
// Keeping the (fragile) parser in ONE place means the two backends can't drift.
import * as piai from "@earendil-works/pi-ai";
import type { AssistantMessage, AssistantMessageEventStream, Context, Model, Tool, ToolCall } from "@earendil-works/pi-ai";
// runtime export, but the package's duplicate `export type` makes TS treat the
// name as type-only — grab the value dynamically
...
export function toolsSection(tools: Tool[] | undefined): string {
...
/** pi Context → OpenAI-shaped messages (system first, then alternating). */
export function toMessages(context: Context): { role: string; content: string }[] {
...
/**
 * Drive a pi AssistantMessageEventStream from an async source of text deltas.
 * `source` is a generator so any awaited setup (engine call / Nano session
 * create) happens inside the try and its errors surface as an error event.
 */
export function makeStream(
  ...
  const base = (): AssistantMessage => ({
    role: "assistant", content: [], api: "openai-completions" as any, provider: provider as any,
    ...
    try {
      ...
      for await (const delta of source()) {
        ...
        if (safe > emitted) {
          ...
          const delta2 = full.slice(emitted, safe);
          emitted = safe;
          stream.push({ type: "text_delta", contentIndex: 0, delta: delta2, partial: partial() });
        }
      }
      ...
      // strip any reasoning block (Qwen3 / DeepSeek-R1 "thinking" models emit
      // <think>…</think> before the answer) so it never leaks into the chat or
      // confuses the tool-call parse
