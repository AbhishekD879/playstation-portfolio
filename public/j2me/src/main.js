...
async function init() {
    ...
    await cheerpjInit({
        ...
        natives: {
            ...
            async Java_pl_zb3_freej2me_bridge_shell_Shell_exit(lib) {
                // AbhishekStation: the console ejects the frame instead of navigating away
                if (window.parent !== window) parent.postMessage({ type: "exit" }, location.origin);
                else { window.close(); setTimeout(() => { document.getElementById('loading').hidden = false; document.getElementById('loading').textContent = 'The game has exited — you can close this tab.'; }, 300); }
            },
    ...
    if (window.__j2meJar) {
        // AbhishekStation: the JAR arrived by postMessage — hand it to CheerpJ as an
        // in-memory file instead of fetching it by URL
        const strPath = '/str/' + window.__j2meJar.name.replace(/[^\w.-]/g, '_');
        cheerpOSAddStringFile(strPath, window.__j2meJar.bytes);
        args = ['jar', strPath];
    } else if (sp.get('app')) {
